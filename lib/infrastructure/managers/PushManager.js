...
class PushManager {
  ...
  async sendToUser(serviceLocator, data) {
    ...
    for (const item of userFirebaseTokens) {
      this.sendToDevice(userNotificationTokensRepository, item.firebaseToken, sendingMessage);
    }
    return { success: true };
  }
  ...
  async sendToUsers(serviceLocator, data) {
    ...
    for (const chunk of chunkArray) {
      sendingMessage.tokens = chunk;
      await this.sendMulticast(sendingMessage);
    }
    return { success: true };
  }

  sendToDevice(userNotificationTokensRepository, firebaseToken, message, options = {}) {
